...
int main(void)
{
    ...
    do{
        while (GameIsValid()) {
            ...
            down_block();
            if (new_block_on == 1) { // down_block()함수에서 바닥에 닿았을경우 새로운 블록을 부르기 위해 new_block_on을 1로 지정한다.
                combo = 0;
                chk = by;
                make_new_block();
                draw_game(SCREEN_UPDATE, SCREEN_MEMORY);
...
void reset_game() {
    ...
    for (i = 1; i < SCREEN_Y - 1; i++) {                     // 사이드 벽 설정
        SCREEN_UPDATE[i][0] = WALL;
        SCREEN_UPDATE[i][SCREEN_X - 1] = WALL;
    }
    memset(&SCREEN_UPDATE[SCREEN_Y - 1][0], WALL, SCREEN_X); // 바닥 벽 설정
}
    ...
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
            if (blocks[b_type][b_rotation][i][j] == 1)
                SCREEN_UPDATE[by + i][bx + j] = ACTIVE_BLOCK; //초기위치인 [bx,by]에서 updata에 저장된 블록 형태 출력
        }
    }
    ...
    while(cnt--) { // 하강까지 대기하며 키입력에 따른 작동
        ...
        switch(key) {
            case IsArrowKey: //방향키입력 -> 방향입력
                ...
                break;
            case SPACE: //블럭 이동
                while (move_block(DOWN)) {} // 아래로 이동할 수 없을 때까지 계속 이동
                for (i = 0; i < SCREEN_Y; i++) { //현재 조작중인 블럭을 굳힘
                    for (j = 0; j < SCREEN_X; j++) {
                        if (SCREEN_UPDATE[i][j] == ACTIVE_BLOCK)
                            SCREEN_UPDATE[i][j] = INACTIVE_BLOCK;}}
                new_block_on = 1; // 새로운 블록 생성 플래그 설정
                break;
            case ESC:
                exit(1);
                break;
